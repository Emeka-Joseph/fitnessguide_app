...
def result_sed():
    #ideets = Categories.query.all()
    readname = db.session.query(Readjustment.srs_name).all()
    readpoint= db.session.query(Readjustment.srs_score).all()
    alldeets = db.session.query(Readjustment).all()
    deets = db.session.query(Readjustment.srs_score).filter(Readjustment.srs_name=='sedentary lifestyle').one()
    
    #udeets = ResultSet.getInt(deets)
    mdeets = db.session.query(Readjustment.srs_id).all()
    if request.method=='GET':
        return render_template('users/result_sedlife.html', deets=deets,mdeets=mdeets,readpoint=readpoint,readname=readname,alldeets=alldeets)
...
def result_social():
    ...
    readname = db.session.query(Readjustment.srs_name).all()
    readpoint= db.session.query(Readjustment.srs_score).all()
    alldeets = db.session.query(Readjustment).filter(Readjustment.srs_name=='social readjustment scale').one()
    deets = db.session.query(Readjustment.srs_score).filter(Readjustment.srs_id==1).one()
    
    #udeets = ResultSet.getInt(deets)
    mdeets = db.session.query(Readjustment.srs_id).all()
    if request.method=='GET':
        return render_template('users/result_social.html', deets=deets,mdeets=mdeets,readpoint=readpoint,readname=readname,alldeets=alldeets)
...
def social():
    ...
    if request.method=='GET':
        ...
        catv = f"UPDATE Readjustment SET srs_score = ({total}) WHERE srs_name = 'social readjustment scale'"
        #sumdeets = "UPDATE Categories SET total_score = (SELECT SUM(cat_point) FROM Categories) WHERE cat_name = 'total'"
